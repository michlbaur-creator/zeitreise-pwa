...
test("legt Episode 3 mit Sprecheraufnahmen im Format von Episode 2 an", async () => {
  ...
  const finalEpisodeQuiz = await readFile(
  ...
  assert.doesNotMatch(episodeOneApp, /<span>Du bist hier<\/span>/);
  assert.doesNotMatch(episodeTwoApp, /<span>Du bist hier<\/span>/);
  assert.doesNotMatch(episodeThreeApp, /<span>Du bist hier<\/span>/);
  assert.match(episodeOneApp, /className="compact-brand-subtitle"/);
  assert.match(episodeTwoApp, /className="compact-brand-subtitle"/);
  assert.match(episodeThreeApp, /compact-brand-subtitle/);
  assert.match(episodeOneApp, /aria-label="Anfang ansehen"/);
  assert.match(episodeTwoApp, /aria-label="Anfang ansehen"/);
  ...
  assert.match(episodeThreeApp, /scenes=\{episodeThreeFinalQuizScenes\}/);
  assert.match(episodeThreeApp, /timeFelsenChallenge/);
  assert.match(episodeThreeApp, /soundMuted=\{soundMuted\}/);
  assert.match(finalEpisodeQuiz, /Die Zeitfelsen-Challenge/);
  assert.match(finalEpisodeQuiz, /Zeitstarter/);
  assert.match(finalEpisodeQuiz, /Spurensucher/);
  assert.match(finalEpisodeQuiz, /Zeitkenner/);
  assert.match(finalEpisodeQuiz, /Zeitmeister/);
  ...
  assert.equal((episodeThreeData.match(/sprecher-und-veo-szene-\d{2}-v1\.m4a/g) ?? []).length, 11);
  ...
  await Promise.all([
    ...
    access(
      new URL(
        "../public/assets/episode3/scene10/bewegung-getreidespeicher-veo-v1.mp4",
        import.meta.url,
      ),
    ),
    access(
      new URL(
        "../public/assets/episode3/scene22/bewegung-kohle-dampfmaschine-veo-v1.mp4",
        import.meta.url,
      ),
    ),
    access(
      new URL(
        "../public/assets/episode3/scene26/bewegung-oel-mobilitaet-veo-v1.mp4",
        import.meta.url,
      ),
    ),
  ]);
  ...
  await Promise.all(
    [
      ...
      "sprecher-und-veo-szene-10-v1.m4a",
      "sprecher-und-veo-szene-22-v1.m4a",
      "sprecher-und-veo-szene-26-v1.m4a",
...
test("aktualisiert Episode 2 und 3 automatisch und ohne Unterbrechung der Sprecheraufnahme", async () => {
  ...
  assert.match(worker, /const CACHE_NAME = "zeitreise-v142"/);
